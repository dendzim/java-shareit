...
public class BookingServiceImplTest {
    ...
    @Test
    public void shouldThrowValidationExceptionWhenUserIsNotOwnerOrBooker() {
        Long userId = 3L; // пользователь, который не является ни владельцем, ни арендатором
        Long bookingId = 1L;

        User booker = new User(1L, "booker", "booker@test.com");
        User owner = new User(2L, "owner", "owner@test.com");

        Item item = new Item(1L, "item", "desc", true, owner.getId(), null);
        ...
        assertThatThrownBy(() -> bookingService.getBooking(userId, bookingId))
                ...
                .hasMessage("Только владелец или арендатор могут посмотреть бронирование");

        verify(bookingRepository).findById(bookingId);
    }

    @Test
    public void shouldThrowNotFoundExceptionWhenUserNotFoundForBooking() {
        BookingDto bookingDto = new BookingDto();
        bookingDto.setItemId(1L);
        bookingDto.setStart(now.plusDays(1));
        bookingDto.setEnd(now.plusDays(2));
        ...
        assertThatThrownBy(() -> bookingService.createBooking(999L, bookingDto))
    ...
    @Test
    public void shouldThrowBadRequestExceptionWhenStartIsAfterEnd() {
        BookingDto bookingDto = new BookingDto();
        bookingDto.setItemId(1L);
        bookingDto.setStart(now.plusDays(2));
        bookingDto.setEnd(now.plusDays(1)); // End before start

        assertThatThrownBy(() -> bookingService.createBooking(1L, bookingDto))
                .isInstanceOf(BadRequestException.class)
                .hasMessage("Начало не может быть после конца бронирования");
